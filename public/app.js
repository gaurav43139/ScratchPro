...
;// CONCATENATED MODULE: ./src/components/Sidebar.js
 ...
 // Assume Icon component is correctly set up
...
function Sidebar(_ref) {
  ...
  const [steps, setSteps] = (0,react.useState)(10);
  const [leftDegrees, setLeftDegrees] = (0,react.useState)(15);
  const [rightDegrees, setRightDegrees] = (0,react.useState)(15);
...
function MidArea(_ref) {
  ...
  const scriptAreaRef = (0,react.useRef)(null);
  ...
  const handleDrop = event => {
    ...
    const dropX = event.clientX - scriptAreaRef.current.getBoundingClientRect().left;
    const dropY = event.clientY - scriptAreaRef.current.getBoundingClientRect().top;
  ...
  const executeScripts = () => {
    scripts.forEach(script => {
      if (script.type === "move") {
        ...
        console.log(`Go to x: ${script.coordinates.x}, y: ${script.coordinates.y}`);
      }
    });
  };
  return /*#__PURE__*/react.createElement("div", {
  ...
  }, "Mid Area"), scripts.map((script, index) => /*#__PURE__*/react.createElement("button", {
    key: index,
    className: "absolute bg-blue-500 p-2 rounded cursor-pointer text-white",
    style: {
      top: script.position.y,
      left: script.position.x
    },
    onClick: () => executeScripts(script)
  }, script.type === "move" && `Move ${script.steps} steps`, script.type === "turnLeft" && `Turn Left ${script.degrees} degrees`, script.type === "turnRight" && `Turn Right ${script.degrees} degrees`, script.type === "goTo" && `Go to x: ${script.coordinates.x}, y: ${script.coordinates.y}`)));
}
...
function CatSprite(_ref) {
  ...
  const [collisionMessage, setCollisionMessage] = (0,react.useState)(null);
  const handleDragStart = event => {
    event.dataTransfer.setData("sprite-id", id);
  };
  const spriteStyle = {
    position: "absolute",
  ...
  const checkCollision = () => {
    ...
    sprites.forEach(otherSprite => {
      if (otherSprite.id !== id) {
        ...
        if (currentSpriteRect.x < otherSpriteRect.x + otherSpriteRect.width && currentSpriteRect.x + currentSpriteRect.width > otherSpriteRect.x && currentSpriteRect.y < otherSpriteRect.y + otherSpriteRect.height && currentSpriteRect.y + currentSpriteRect.height > otherSpriteRect.y) {
          collisionDetected = true;
          collidedSpriteIds.push(otherSprite.id);
        }
      }
    });
    if (collisionDetected) {
      setCollisionMessage(`Collision detected with sprites: ${collidedSpriteIds.join(', ')}`);
    } else {
...
function PreviewArea(_ref) {
  ...
  const [isSpriteVisible, setIsSpriteVisible] = (0,react.useState)(false);
  ...
  const handleDrop = event => {
    ...
    setSprites(prevSprites => prevSprites.map(sprite => sprite.id === spriteIdNumber ? {
      ...sprite,
  ...
  }, isSpriteVisible ? "Hide Sprite" : "Show Sprite"), sprites.map(sprite => {
...
function App() {
  ...
  return /*#__PURE__*/react.createElement("div", {
    ...
    rotation: sprites.find(sprite => sprite.id === selectedSpriteId)?.rotation,
    position: sprites.find(sprite => sprite.id === selectedSpriteId)?.position
